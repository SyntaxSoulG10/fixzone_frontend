 ...
 */
export async function initPayment(servicePackageId: string, vehicleId: string, date: string, timeSlot: string): Promise<number> {
  const res = await fetch(`${BASE_URL}/payments/init`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ servicePackageId, vehicleId, date, timeSlot }),
  });
