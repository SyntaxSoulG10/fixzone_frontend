...
export default function BookServicePage() {
  ...
  const loadCenters = async (lat?: number, lng?: number, radius?: number) => {
    ...
    try {
      ...
      console.log(`[BookServicePage] GET ${url} → status ${res.status}`);
      ...
      const visibleCenters = list.filter((c) => {
        const status = String(c.status ?? "").toUpperCase();
        const isActive = c.isActive !== false;
        const paymentReady = isPaymentReady(c);
        return status === "APPROVED" && isActive && paymentReady;
      });
